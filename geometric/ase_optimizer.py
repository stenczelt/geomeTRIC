...
import geometric
from .optimize import OptParams, ang2bohr, OPT_STATE, setup_coordinate_system
...
class GeomeTRIC(Optimizer):
    ...
    limitations:
    - fmax in self.run() is not understood, uses the internal convergence
    thresholds or anything set on construction
    - GeomeTRIC's constraints are not implemented, if ASE is modifying the
    forces then it may work, the plan is to pull the constraints from ASE and
    convert them to the GeomeTRIC internal ones, or just use the constraint
    input of GeomeTRIC and raise errors if the Atoms object has any constraints
    ...
    def __init__(
        ...
        **kwargs
        ...
        # IC
        self.IC = setup_coordinate_system(
            self.molecule,
            coordsys=kwargs.get("coordsys", "tric"),
            engine=self.engine,
            conmethod=self.opt_params.conmethod,
            dirname=None,
            bothre=self.opt_params.bothre,
            CVals=self.constraint_values,
            Cons=self.constraint_coordinates,
        )
        ...
        self.optimizer = geometric.optimize.Optimizer(
            self.molecule.xyzs[0].flatten() * ang2bohr,
            self.molecule,
            self.IC,
            self.engine,
